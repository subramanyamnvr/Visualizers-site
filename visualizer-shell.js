...
if(/cloud\/cloud-platforms-comparison\//i.test(locationPath)){return{mountSelector:".shell",title:"Cloud Platforms Comparison Atlas",copy:"Move from a focused workbench into provider comparisons, service lenses, and official references.",maxWidth:"1280px",pages:[{id:"workbench",label:"Workbench",group:"Overview",selectors:["#workbench"]},{id:"home",label:"Home",group:"Overview",selectors:[".hero"]}],autoSelector:"section.panel[id]",autoGroup:"Topics"};}
if(/cloud\/aws-cloud-services\//i.test(locationPath)){return{mountSelector:".shell",title:"AWS Cloud Services",copy:"Use the workbench first, then move into the full AWS service catalog and official references.",maxWidth:"1320px",pages:[{id:"workbench",label:"Workbench",group:"Overview",selectors:["section#workbench"]},{id:"home",label:"Home",group:"Overview",selectors:[".hero"]}],autoSelector:"section.panel[id]",autoGroup:"Topics"};}
if(/cloud\/gcp-cloud-services\//i.test(locationPath)){return{mountSelector:".shell",title:"Google Cloud Services",copy:"Use the workbench first, then move into the full Google Cloud service catalog and official references.",maxWidth:"1320px",pages:[{id:"workbench",label:"Workbench",group:"Overview",selectors:["section#workbench"]},{id:"home",label:"Home",group:"Overview",selectors:[".hero"]}],autoSelector:"section.panel[id]",autoGroup:"Topics"};}
if(/data-engineering\/azure-cloud-services\//i.test(locationPath)){return{mountSelector:".shell",title:"Azure Cloud Services",copy:"Start with a workbench that narrows the catalog by workload, then move into category study and the live service explorer.",maxWidth:"1320px",pages:[{id:"workbench",label:"Workbench",group:"Guide",selectors:["#workbench"]},{id:"home",label:"Overview",group:"Guide",selectors:[".hero",".method"]},{id:"categories",label:"Category Studio",group:"Guide",selectors:[".category-panel"]},{id:"explorer",label:"Service Explorer",group:"Explore",selectors:[".explorer",".footer"]}]};}
if(/well-architected-frameworks\//i.test(locationPath)){return{mountSelector:".shell",title:"Well-Architected Atlas",copy:"Start from a focused workbench, then step into the full framework map one pillar at a time.",maxWidth:"1080px",pages:[{id:"workbench",label:"Workbench",group:"Overview",selectors:["#workbench"]},{id:"home",label:"Home",group:"Overview",selectors:[".hero"]}],autoSelector:"section.section-card[id]",autoGroup:"Topics"};}
if(/system-design\/ai-workbench\//i.test(locationPath)){return{mountSelector:".shell",title:"System Design Atlas",copy:"Study one system-design idea at a time and keep the bigger map in the left rail.",maxWidth:"1100px",pages:[{id:"workbench",label:"Workbench",group:"Overview",selectors:[".workbench-grid"]},{id:"home",label:"Home",group:"Overview",selectors:[".hero"]}],autoSelector:"section.section-card[id]",autoGroup:"Topics"};}
return null;}
...
function buildPrimerBullets(title,description,target){const primer=buildPrimerData(title,description);const category=cleanText(definition.category).toLowerCase();const isMachineLearning=category.includes("machine learning");const primerItems=isMachineLearning?[...extractTopicContentBullets(target),...buildPrimerDepthBullets(title,description),primer.why,...(Array.isArray(primer.watch)?primer.watch:[])]:[primer.simple,primer.analogy,...extractTopicContentBullets(target),...buildPrimerDepthBullets(title,description),...(Array.isArray(primer.steps)?primer.steps:[]),primer.why,...(Array.isArray(primer.watch)?primer.watch:[])];return dedupeStrings(primerItems).map(cleanText).filter(Boolean).slice(0,11);}
...
function renderReferenceLinks(references){return(Array.isArray(references)?references:[]).filter((reference)=>reference&&reference.url).map((reference)=>`
        ...
        `).join("");}
function buildPrimerDepthBullets(title,description){const category=cleanText(definition.category).toLowerCase();const intro=firstSentence(description);if(category.includes("machine learning")){const topic=cleanText([title,description].join(" ")).toLowerCase();const isClassification=/classification|logistic regression|precision|recall|f1|roc|auc|spam|fraud|screening/.test(topic);const isRegression=/regression|prediction|forecast|estimate|continuous/.test(topic)&&!isClassification;const isClustering=/cluster|clustering|segment|segments|unsupervised/.test(topic);if(isClassification){return["Classifiers are best read by the tradeoff they make between false positives, false negatives, and the threshold that separates the two.","Fresh validation data matters more than a polished training score when you want to trust a labeler in real use."];}
if(isRegression){return[`${title} becomes easier to judge when you check residuals, error spread, and whether the model still behaves on fresh examples.`,"A baseline and a clean holdout split matter more than a fancier curve fit that only looks good in training."];}
if(isClustering){return[`${title} is easiest to trust when you ask whether the groups are stable, separable, and useful for a real downstream decision.`,"Cluster shape, scale, and feature choice matter more than a prettier plot."];}
return[intro&&!sameMeaning(intro,title)?`${intro} Keep the evaluation split clean so the score reflects fresh examples instead of memorized noise.`:"",`${title} becomes easier to judge when you ask four things in order: what goes in, what pattern it can learn, what score proves it works, and what mistake would hurt most in real use.`];}
