...
function cacheDom(){["brandMark","brandTitle","brandSubtitle","heroEyebrow","heroTitle","heroBody","visualLead","overviewLead","catalogLead","catalogFootnote","journeyLead","referenceLead","globalFootnote","heroStats","overviewGrid","legendGrid","categoryList","serviceSearch","domainSelect","catalogSummary","serviceGrid","detailPanel","journeyGrid","referenceGrid","constellationCanvas"].forEach((id)=>{ui[id]=document.getElementById(id);});}
...
function renderAll(){renderHeroStats();renderOverview();renderLegend();renderCategoryFilters();renderDomainOptions();renderCatalog();renderJourneys();renderReferences();}
function renderHeroStats(){if(!ui.heroStats||!state.catalog){return;}
const topDomain=findTopDomain(state.catalog.domainTotals);const referenceCount=(state.catalog.docsFromPageCount||0)+(state.catalog.docsSearchFallbackCount||0);const coverage=state.catalog.products.length?Math.round((referenceCount/state.catalog.products.length)*100):0;const stats=[{value:formatNumber(state.catalog.categoryCount||state.catalog.displayCategories.length),label:"Official categories",note:"Directly grounded in the vendor's published cloud catalog."},{value:formatNumber(state.catalog.uniqueServiceCount||state.catalog.products.length),label:"Cataloged services",note:"Each service keeps its product page and documentation path nearby."},{value:topDomain.label,label:"Largest operating band",note:"The densest cluster in this platform map right now."},{value:coverage+"%",label:"Reference coverage",note:"Products with a documentation path surfaced from official pages."}];ui.heroStats.innerHTML=stats.map((item)=>`
